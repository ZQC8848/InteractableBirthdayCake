...
- **无 LiDAR 的机型照样能拿到手的深度，不要以为只能退化成平面 raycast。** `personSegmentationWithDepth` 会通过 `ARFrame.estimatedDepthData` 给出神经网络推断的深度，**仅对被分割为「人」的像素有效**——手正属于这一类。要求 A12 及以上、不需要 LiDAR。降级链在 `Hand/HandDepthSource.swift`：LiDAR 优先，没有就用它。注意 `estimatedDepthData` 在人体分割蒙版之外**也会输出数值，但那些值没有意义**，采样时必须用 `segmentationBuffer` 过滤掉。
  LiDAR 本身从 iPhone 12 起就是 Pro 系列独占：标准版和 Plus（包括 iPhone 16 Plus）都没有。
